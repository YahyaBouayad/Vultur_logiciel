...
from schemas.bon_commande import BonCommandeCreate, BonCommandeOut, StatutCommandeUpdate
...
def create_bon(data: BonCommandeCreate, db: Session = Depends(get_db), _: Utilisateur = Depends(get_current_user)):
    ...
    bc = BonCommande(fournisseur_id=data.fournisseur_id, notes=data.notes, statut="brouillon")
    db.add(bc)
    db.flush()
